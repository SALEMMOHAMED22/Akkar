...
<div class="container py-4">
    ...
    <h2 class="mb-4">{{ app()->getLocale() == 'ar' ? $ad->title_ar : $ad->title_en ?? 'غير محدد' }) }}</h2>
    ...
    <div class="row">
                    ...
                    <div class="carousel-inner">
                        @foreach($ad->images as $key => $image)
                            <div class="carousel-item {{ $key === 0 ? 'active' : '' }}">
                                <img src="{{ asset('storage/' . $image->path) }}" class="d-block w-100" alt="Ad Image">
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="col-md-6">
            ...
            <ul class="list-group mb-3">
                <li class="list-group-item"><strong>التصنيف:</strong> {{ $ad->adCategory->name ?? 'غير محدد' }}</li>
                <li class="list-group-item"><strong>التصنيف الفرعي:</strong> {{ $ad->adSubCategory->name ?? 'غير محدد' }}</li>
                <li class="list-group-item"><strong>التصنيف الفرعي الثاني:</strong> {{ $ad->adSubSubCategory->name ?? 'غير محدد' }}</li>
                <li class="list-group-item"><strong>المعلن:</strong> {{ $ad->user->name ?? 'غير معروف' }}</li>
                <li class="list-group-item"><strong>الوصف:</strong> {{ $ad->description }}</li>
