            ...
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                ...
                @if (auth('admin')->user())
                    <form action="{{ route('dashboard.ads.updateStatus', $ad->id) }}" method="POST"
                        ...
                        @csrf
                        @method('PUT')
                        <select name="status" class="form-select form-select-sm me-2" id="statusSelect">
        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body p-4 mt-3 ">
                <div class="d-flex align-items-center mb-3">
                    <img src="{{ $ad->user->image ? asset($ad->user->image) : asset('default.png') }}"
                    ...
                    <div class="row">
                        @foreach ($ad->images as $img)
                            <div class="col-md-3 mb-3">
                                <img src="{{ asset($img->image) }}" class="img-fluid rounded border shadow-sm mt-3"
                ...
                <div class="card-body">
                    <div class="row">
                        @foreach ($ad->userWorks as $work)
                            <div class="col-md-3 mb-3">
                                <img src="{{ asset($work->image) }}" class="img-fluid rounded border shadow-sm mt-3"
                                    style="width: 100%; height: 200px; object-fit: cover;">
                            </div>
                        @endforeach
                    </div>
    ...
    <script>
        document.getElementById('statusSelect').addEventListener('change', function() {
            if (this.value === 'rejected') {
                ...
                // إرسال الطلب مباشرة لتغيير الحالة (مقبول أو معلق)
                fetch("{{ route('dashboard.ads.updateStatus', $ad->id) }}", {
                    method: 'POST',
                    headers: {
