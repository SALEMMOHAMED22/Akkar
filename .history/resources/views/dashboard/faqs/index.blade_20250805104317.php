...
@section('content')
    ...
    <h2 class="accordion-header" id="heading_{{ $item->id }}">
        <button class="accordion-button @if (!$loop->first) collapsed @endif bg-light fw-semibold"
    ...
    <div id="collapse_{{ $item->id }}" class="accordion-collapse collapse @if ($loop->first) show @endif"
        ...
        <div class="accordion-body position-relative">
            <p class="mb-2">{{ app()->getLocale() == 'ar' ? $item->answer_ar : $item->answer_en }}</p>
            <div class="position-absolute end-0 bottom-0 m-2">
                <a faq-id="{{ $item->id }}" class="btn btn-sm btn-outline-danger me-2 delete_confirm_btn">
