...
if (!function_exists('subscription_check')) {
    ...
    function subscription_check( string $feature, int $qty = 1): array
    {
        $user = auth('sanctum')->user();

        if (!$user) {
            return ['ok' => false, 'remaining' => 0, 'reason' => 'no_user'];
        }
        $userId = $user->id;
        ...
        switch ($feature) {
            ...
            case 'vr':
                $limit = (int) $limits->vr_tours_limit;    
