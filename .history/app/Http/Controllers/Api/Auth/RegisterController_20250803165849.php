<?php

namespace App\Http\Controllers\Api\Auth;

use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Http\Requests\UserRequest;
use App\Http\Requests\VerifyRequest;
use App\Http\Resources\UserResource;
use App\Http\Resources\UserVerifyResource;
use App\Interfaces\Auth\AuthRepositoryInterface;

class RegisterController extends Controller
{
    protected $authRepo;

    public function __construct(AuthRepositoryInterface $authRepo)
    {
        $this->authRepo = $authRepo;
    }

 
    public function register(UserRequest $request)
    {

        $resu = $this->authRepo->register($request->validated());
        if (! $user) {
            return apiResponse(400, 'User registration failed');
        }

        return apiResponse(200, 'User registered successfully', ['email' => $user->email , 'code' => $otp['otp'] ]  );
    }

    public function verifyEmail(VerifyRequest $request)
    {

        $data = $this->authRepo->verifyEmail($request->email, $request->code);

        if ($data === null) {
            return apiResponse(404, 'User not found');
        }

        if ($data === false) {
            return apiResponse(400, 'Invalid or expired OTP');
        }

        // dd($data);
        return apiResponse(200, 'Email verified successfully', [
            'user' => new UserVerifyResource($data['user']),
            'token' => $data['token'],
        ]);
    }

    public function resendOtp(Request $request){
            $request->validate([
                'email' => 'required|email|exists:users,email',
            ]);

        $user = $this->authRepo->resendOtp($request->email);
        // dd($user);

        if (!$user) {
            return apiResponse('400', 'User not found');
        }else{
            return apiResponse(200 , 'OTP sent successfully' , ['email' => $user->email]);
        }


    }
}
