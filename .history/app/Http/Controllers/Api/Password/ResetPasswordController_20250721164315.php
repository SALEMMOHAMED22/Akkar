...
use App\Http\Requests\ResetPasswordRequest as RequestsResetPasswordRequest;
...
class ResetPasswordController extends Controller
{
    ...
    public function resetPassword(ResetPasswordRequest $request){
        $user = $this->resetPassRepo->resetPassword($request->email , $request->password);
        if (!$user) {
            return apiResponse(400, 'User not found');
        }else{
            return apiResponse(200 , 'Password reset successfully' , ['email' => $user->email]);
        }
    }){
