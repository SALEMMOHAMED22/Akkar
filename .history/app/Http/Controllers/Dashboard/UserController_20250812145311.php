<?php

namespace App\Http\Controllers\Dashboard;

use App\Models\User;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Yajra\DataTables\DataTables;

class UserController extends Controller
{
    public function showUserIndividual()
    {

        return view('dashboard.users.individual');
    }

    public function showUserCompany()
    {

        return view('dashboard.users.company');
    }

    public function usersIndividual()
    {
        $usersIndividual = User::with(['jobtitle', 'identifies'])->where('type', 'individual')->get();

        // dd($usersIndividual);
        return DataTables::of($usersIndividual)
            ->addIndexColumn()
            ->addColumn('type', function ($user) {
                return $user->type;
            })
            ->addColumn('name', function ($user) {
                return $user->name;
            })
            ->addColumn('email', function ($user) {
                return $user->email;
            })
            ->addColumn('phone', function ($user) {
                return $user->phone;
            })
            ->addColumn('age', function ($user) {
                return $user->age;
            })
            ->addColumn('job_title', function ($user) {
                return app()->getLocale() == 'ar' ? $user->jobtitle->name_ar : $user->jobtitle->name_en;
            })
            ->addColumn('national_id', function ($user) {
                return $user->national_id;
            })
            ->addColumn('created_at', function ($user) {
                return $user->created_at->format('Y-m-d');
            })
            ->editColumn('email_notification', function ($user) {
                return $user->email_notification ? $user->email_notification : 'Not Set';
            })
            ->addColumn('is_active', function ($user) {
                return $user->getStatusTranslated();
            })
            ->addColumn('image', function ($user) {
                $url = $user->image ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('personal_photo', function ($user) {
                $url = $user->identifies?->personal_photo ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('national_id_front', function ($user) {
                $url = $user->identifies?->national_id_front ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('national_id_back', function ($user) {
                $url = $user->identifies?->national_id_back ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('engineer_card_front', function ($user) {
                $url = $user->identifies?->engineer_card_front ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('engineer_card_back', function ($user) {
                $url = $user->identifies?->engineer_card_back ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })

            ->addColumn('action', function ($user) {
                return view('dashboard.users.actions', compact('user'))->render();
            })
            ->rawColumns(['action', 'image',  'personal_photo', 'national_id_front', 'national_id_back', 'engineer_card_front', 'engineer_card_back'])
            ->make(true);
    }


    public function usersCompany()
    {
        $usersCompany = User::with(['companytype', 'identifies'])
            ->where('type', 'company')
            ->get();

        return DataTables::of($usersCompany)
            ->addIndexColumn()
            ->addColumn('type', fn($user) => $user->type)
            ->addColumn('company_name', fn($user) => $user->company_name ?: '-')
            ->addColumn('email', fn($user) => $user->email ?: '-')
            ->addColumn('phone', fn($user) => $user->phone ?: '-')
            ->addColumn('national_id', fn($user) => $user->national_id ?: '-')
            ->addColumn('company_type', function ($user) {
                return app()->getLocale() == 'ar'
                    ? ($user->companytype->name_ar ?? '-')
                    : ($user->companytype->name_en ?? '-');
            })
            ->editColumn('email_notification', fn($user) => $user->email_notification ?: 'Not Set')
            ->addColumn('created_at', fn($user) => $user->created_at->format('Y-m-d'))

            ->addColumn('image', function ($user) {
                $url = $user->image ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('company_logo', function ($user) {
                $url = $user->identifies?->company_logo ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('tax_record_front', function ($user) {
                $url = $user->identifies?->tax_record_front ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('tax_record_back', function ($user) {
                $url = $user->identifies?->tax_record_back ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('tax_card_front', function ($user) {
                $url = $user->identifies?->tax_card_front ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })
            ->addColumn('tax_card_back', function ($user) {
                $url = $user->identifies?->tax_card_back ?: 'default.png';
                return view('dashboard.datatable.image', ['url' => $url])->render();
            })

            ->addColumn('action', fn($user) => view('dashboard.users.actions', compact('user'))->render())

            ->rawColumns([
                'action',
                'image',
                'company_logo',
                'tax_record_front',
                'tax_record_back',
                'tax_card_front',
                'tax_card_back'
            ])
            ->make(true);
    }



    public function changeStatus(Request $request)
    {
        $user = User::find($request->id);

        $user->is_active == 1 ? $status = 0 : $status = 1;

        $user =  $user->update([
            'is_active' => $status
        ]);

        if (!$user) {
            return response()->json([
                'status' => 'error',
                'message' => __('dashboard.error_msg'),
            ], 500);
        }


        return response()->json([
            'status' => 'success',
            'message' => __('dashboard.status_changed_successfully'),
        ], 200);
    }
}
