...
{ 
    ...
    */
    ...
    {
        if($id == 1){
            return redirect()->back()->with('error', 'You can not delete this admin');
        }
        $admin = Admin::findOrFail($id);
        if(! $admin){
            return redirect()->back()->with('error', 'Admin not found');
    ...
    public function changeStatus($id){
        if($id == 1){
            return redirect()->back()->with('error', 'You can not change status of this admin');
        }
        ...
        $admin->save();
        return redirect()->route('dashboard.admins.index')->with('success', 'Admin status changed successfully');
    }
}
