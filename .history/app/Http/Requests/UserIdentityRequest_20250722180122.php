...
class UserIdentityRequest extends FormRequest
{
    ...
    if ($user->type === 'individual') {
        $rules = [
            ...
            'personal_photo'         => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
            'national_id_front'      => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
            'national_id_back'       => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
            'engineer_card_front'    => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
            'engineer_card_back'     => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
        ];
    } elseif ($user->type === 'company') {
        $rules = [
            'tax_number'             => ['required', 'string', 'max:50'],
            'company_logo'           => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
            'tax_record_front'       => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
            'tax_record_back'        => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
            'tax_card_front'         => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
            'tax_card_back'          => [$isFirstTime ? 'required' , 'image', 'mimes:jpg,jpeg,png', 'max:2048'],
        ];
    }
