...
class PlanResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'price'     => number_format((float)$this->price, 2) . __('response.pound'),
            'ads_limit'         => $this->ads_limit ?? 0 . __('response.ads'),
            'images_limit'             => $imagesText,
            'video'           => $this->video ? '✅' : '✖',
            'vr_tours'          => (int) $this->vr_tours . __('response.vr_tours'),
             'search_priority'  => $searchMap[$this->search_priority] ?? $this->search_priority,
            'reports'          => $reportsMap[$this->reports] ?? $this->reports,
            'team_members'        => $this->team_members > 0 ? ('حتى ' . $this->team_members . ' مستخدم') : 'لا',
        ];
    }
}
