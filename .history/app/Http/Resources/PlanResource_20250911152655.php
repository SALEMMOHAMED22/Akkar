...
class PlanResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
          // خرائط نصوص الجدول
        $searchMap = [
            'normal'      => 'ظهور عادي تسلسلي',
            'highlighted' => 'ظهور مميّز',
            'top'         => 'الأعلى ظهورًا',
        ];
        ...
        // الصور غير محدودة لو images_limit كبيرة أو null
        $imagesText = ($this->images_limit === null || $this->images_limit >= 100000)
            ? 'عدد غير محدود ✅'
            : (string) $this->images_limit;

        return [
            'الباقة'            => $this->name, // يعتمد على accessor getNameAttribute() لو عاملُه
