...
class AdRepository implements AdRepositoryInterface
{
    ...
    public function getAdsByCatId($catId){
        $ads = Ad::with(['adCategory','adSubCategory','adSubSubCategory','user','images','userWorks','adFiles'])->where('ad_category_id', $catId)->latest()->paginate(9);
        if(! $ads){
            return false;
        }
        return $ads;
