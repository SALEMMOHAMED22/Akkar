...
class AdRepository implements AdRepositoryInterface
{
    ...
    public function storeReview(array $data)
    {
        ...
        $ad = Ad::find($adId);
        if (! $ad) {
            return ['status' => 'error' , 'message' => 'Ad not found' , 'code' => 400];
        }
        ...
        if (! $review) {
            return ['status' => 'error' , 'message' => 'An unexpected error occurred while creating the review.' , 'code' => 400];
        }
        // dd($review);

        return ['status' => 'success' , 'message' => 'Review created successfully' , 'code' => 200 , 'data' => $review->load(['user'])];
    }
