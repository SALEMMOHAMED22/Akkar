...
class PropertyRepository implements PropertyRepositoryInterface
{
    ...
    {
        return Property::with(['images', 'files', 'user'])
    ...
    public function show($id)
    {
        ...
        $prop = Property::with(['images', 'files', 'user'])
            ->where('id', $id)
            ->first();
    ...
    public function store(array $data)
    {
        try {
            ...
            return DB::transaction(function () use ($user, $data) {
                ...
                if (!empty($data['files'])) {
                    $uploadFiles = ImageManger::uploadMultiFile('/storage/properties', $data['files']);
                    foreach ((array) $uploadFiles as $file) {
                        $property->files()->create(['file' => $file['path']]);
                    }
                }
