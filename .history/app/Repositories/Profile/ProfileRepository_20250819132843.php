...
class ProfileRepository implements ProfileInterface
{
    ...
    public function updateProfile($data)
    {
        ...
        if (isset($data['image'])) {
            ...
            $path =  ImageManger::uploadImage('storage/user_profile', $image, 'public');
    ...
    public function identifiesStore(array $data)
    {
        ...
        foreach ($imageFields as $field) {
            $imagesToUpload[$field] = isset($data[$field])
                ? ImageManger::uploadImage('storage/user_identifies', $data[$field], 'public')
                : null;
        }
    ...
    public function identifiesUpdate(array $data)
    {
        ...
        if (!$identifies) {
           return ['status' => 'error', 'message' => 'Identifies not found', 'code' => 404];
        }
        ...
        foreach ($imageFields as $field) {
            if (isset($data[$field]) && !empty($data[$field])) {
                ...
                $updatedImages[$field] = ImageManger::uploadImage('storage/user_identifies', $data[$field], 'public');
            }
        }
    ...
    public function getProviderProfile($providerId)
    {
        ...
        $providerProfile = User::with([
            'ads.images' => fn($q) => $q->limit(1),
            'ads.reviews',
        ])
            ->withCount('ads')
    ...
    public function getUserIdentifies()
    {
        $user = auth('sanctum')->user();

        $userIdentifies = $user->identifies;
        // dd($userIdentifies);
